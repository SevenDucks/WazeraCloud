...
public class DocumentJpaRepositoryImpl implements DocumentJpaRepository {
	...
	public List<Document> findByTags(List<String> searchTokens) {
		...
		for (int i = 0; i < searchTokens.size(); i++) {
			queryBuilder.addWhere("(doc.name like :textSearchToken" + i + " or exists (select documentTag from DocumentTag documentTag where doc.id = documentTag.documentId and documentTag.value like :searchToken" + i + "))");
		}
